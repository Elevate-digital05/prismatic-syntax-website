/* The package cards and a since-removed payment calculator used to keep
   separate hand-written copies of every feature list, and they drifted — the
   calculator sold Pro clients a "Dedicated Account Manager" that nobody was
   employed to be. The cards then spent a while being rendered by JavaScript on
   load, which shipped four empty <ul>s to the crawler on the one page that has
   to rank.

   The cards are now baked into south-africa.html by build.mjs from
...
for (const [name, pkg] of Object.entries(PACKAGES)) {
  ...
  const cardItems = [...between(`feats-${name}`).matchAll(/<li>(.*?)<\/li>/gs)].map(m => strip(m[1]));
...
const promises = Object.values(PACKAGES).flatMap(p => p.features.flat());
...
for (const [, name] of html.matchAll(/class="p-card[^"]*"[^>]*data-plan="([^"]+)"/g)) {
...
console.log(`ok packages: ${total} features baked into the package cards from lib/packages.js`);
